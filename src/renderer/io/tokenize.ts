...
export default function tokenize(input: string): Token[] {
  ...
  while (currentPosition < input.length) {
    ...
    if (currentToken === '{' && openBrackets < 2) {
      // openBrackets>=2 must belong to a field value
      openBrackets++;
      currentPosition++;
      continue;
    ...
    if (openBrackets > 1) {
      ...
      while (openBrackets > 1) {
        ...
        else if (bracket === '}') openBrackets--;
        if (openBrackets > 1) parts.push(bracket);
      }
