...
const PROPERTY_LENGTH = Math.max.apply(
  Math,
  WRITE_ORDER.map((x) => x.length)
);
...
export default function renderBibtex(entry: Entry) {
  ...
  for (const propertyName of WRITE_ORDER) {
    if (propertyName in entry) {
      buffer.push(formatProperty(propertyName, entry[propertyName]));
    }
  }
  for (const propertyName of Object.keys(entry).filter(remainingKeyFilter)) {
    console.log(
      `WARNING: the WRITE_ORDER of the property '${propertyName}' is unknown`
    );
    buffer.push(formatProperty(propertyName, entry[propertyName]));
  }
  buffer.push('\n}\n');
